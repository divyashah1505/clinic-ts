...
const { initSocket, sendNotificationToUser } = require("../controllers/socketController")
import type { Request, Response } from "express"
  ...
  const   login = async (req: Request, res: Response) => {
   ...
   const  getDoctorAvailableSlots = async (req: Request, res: Response) => {
        try {
            ...
            const selectedDate = new Date(year as number, month as number - 1, day);
            const dayName = selectedDate.toLocaleDateString('en-US', { weekday: 'long' });

            const dayMap = settings.wokringHours.find((m:any) =>
            ...
            console.log(doctors)
            const startOfDay = new Date(year as number, month as number - 1, day, 0, 0, 0);
            const endOfDay = new Date(year as number, month as number - 1, day, 23, 59, 59);

            const results = await Promise.all(doctors.map(async (doc:any) => {
                ...
                timeRanges.forEach((range:any) => {
                    ...
                    while (current < endLimit) {
                        ...
                        const isBooked = booked.some((a:any) => a.startTime >= slotStart && a.startTime < slotEnd);
  ...
  const  bookAppoitments = async (req: any, res: Response) => {
        try {
            ...
            const patientId = new mongoose.Types.ObjectId(userId); const { doctorId, appointmentDate, startTime, endTime } = req.body;
            ...
            const settings = await AdminSetting.findOne();
            const dayConfig = settings.wokringHours.find((m:any) =>
                (m instanceof Map ? m.get('day') : m.day) === dayName
            );

            if (!dayConfig) return error(res, `Doctor does not work on ${dayName}`);

            const formatTime: any = (time: any) => !time.includes(":") ? `${time.padStart(2, "0")}:00` : time;
            ...
            const appointmentStart: any = new Date(selectedDate);
            appointmentStart.setHours(startH, startM, 0, 0);
            let newDate: any = new Date()
            if ((appointmentStart - newDate) / (1000 * 60 * 60) < 3) {
            ...
            sendNotificationToUser(doctorId, 'Booked Appoitments', {
                message: appString.NEWAPPOITMENTBOOKED,
    ...
    const cancelAppointment = async (req: any, res: Response) => {
        try {
            const userId = req.user?.id;
            req = req as Request
            ...
            const start: any = new Date(appoint.appointmentDate);
            ...
            await appoint.save();
            sendNotificationToUser(patientId, 'cancel Appointmet', {
                message: appString.APPOITMENTCANCELLED,
    ...
    const rescheduleAppointment = async (req: any, res: Response) => {
        try {
            const { appoitmentId } = req.params;
            const userId = req.user?.id;
            req = req as Request
            ...
            if (appoint.cancelBy != patientId) {
                return error(res, { message: appString.APPOITMENT_NOT_FOUND });
            }

            const oldStart:any = new Date(appoint.appointmentDate);
            const [h, m] = appoint.startTime.split(":").map(Number);
            oldStart.setHours(h, m);
            let newDate:any = new Date()
            if ((oldStart - newDate) / (1000 * 60) < 60) {
...
export default {patientRegister,login,verifyOtpLogin,getDoctorAvailableSlots,bookAppoitments,cancelAppointment,rescheduleAppointment,getAppointments,getAppointmentDetails};
